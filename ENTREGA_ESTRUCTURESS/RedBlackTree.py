# -*- coding: utf-8 -*-
"""
Created on Sat Nov 30 11:17:59 2019
...
class ValorRDT():
    ...
    def __str__(self):
        return str(self._key)+" "+str(self._valor)
...
class RedBlackTree:
    ...
    def maxim(self):   
        nodeAct=self.root
        while nodeAct!=None:            
            if nodeAct.right!=self.NIL_LEAF: nodeAct=nodeAct.right
            else:
                return nodeAct.value
    ...
    def find_node(self, value):
        def inner_find(root):
            if root is None or root == self.NIL_LEAF:
                return None
            #print("FIND_NODE:REDBLACKTREE",value,root.value)
            if value > root.value:
